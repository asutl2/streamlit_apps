...
if uploaded_files:
   ...
   #機械学習のタイプを選択する。
   ml_menu = st.selectbox("実施する機械学習のタイプを選択してください", ["重回帰分析","ロジスティック回帰分析", "XGBRegressor", "XGBClassifier"])
   ...
   if ml_menu == "重回帰分析":
           ...
           if execute:
                 # カテゴリー変数をダミー変数化
                 categorical_features = [] # カテゴリー変数のカラム名を入れる
                 if categorical_features:
                     df_ex = pd.get_dummies(df[ex], columns=categorical_features)
                 else:
                     df_ex = df[ex]
                 df_ob = df[ob]
    ...
    # XGBClassifierを選択した場合
        ...
        if execute:
            ...
            # 正解率を表示
            col1, col2 = st.columns(2)  
            col1.metric(label="トレーニングスコア", value=xgb_clf.score(X_train, y_train))
            col2.metric(label="テストスコア", value=xgb_clf.score(X_test, y_test))
